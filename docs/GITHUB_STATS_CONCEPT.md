...
2. **The history lives in the Neovim config on purpose** —
   ...
   not be written into that folder (see [D2](#d2--two-locations-history-synced-digest-local)).
...
| `require("github_stats")` loads the dashboard at module load, and the dashboard needs `ui.nvim` | `github_stats.nvim` (`init.lua`, README "Around it") | Built — why the probe in [D2](#d2--two-locations-history-synced-digest-local) targets a smaller module |
...
### D1 — the plugin publishes a digest; the readers do not re-derive it
...
So, after each fetch, `github_stats.nvim` writes one **digest file per repo**
(where: [D2](#d2--two-locations-history-synced-digest-local)):
...
Fallback for a plugin that has not written a digest yet (older version, never
...
the app, and dropped as soon as a digest exists.
...
### D2 — two locations: history synced, digest local
...
`digest.json` is different: one file, rewritten on every fetch, **derived** from
...
- `stdpath("data")/github_stats.nvim/root.json` — `{ "schema": 1, "digest_dir": "...", "data_dir": "...", "repos": { "owner/name": "owner_name" } }`
- `stdpath("data")/github_stats.nvim/digest/<owner_name>.json`
...
files elsewhere. So `github_stats.nvim` gets one option, `digest_dir`
(default `stdpath("data")/github_stats.nvim`), and always writes the tiny
`root.json` pointer to the *default* place, naming wherever `digest_dir` really
is. Digests always sit in a `digest/` subfolder of `digest_dir`, so even a
user who points it at their history directory never has a digest next to the
per-repo history folders.

Each reader finds the digest through the same **discovery chain**, first hit
wins:

1. **An explicit setting.** In the app: the Traffic section has a **folder button**
   (the native folder dialog, which the app already has) to choose the digest
   ...
   `documentation.nvim`: `opts.traffic.digest_dir` in the plugin's own setup
   spec.
2. **`root.json`** at the default place.
3. **Ask Neovim.**
   - `documentation.nvim`: `soft_require.probe("github_stats.digest")` — a
     **small module of its own**, not `require("github_stats")`. The plugin's
     ...
     UI dependency. `github_stats.digest` has no UI requires and answers
     `digest_dir()` even when `setup()` has not run (it then returns the
   ...
   - The app: the same question, asked the way the spec import already asks
     Neovim things — `nvim --headless -c "luafile <script>"` through the `nvim`
     path the app already has a setting for (`import_from_nvim_config` is the
     template, including its `spawn_blocking`). One process start, only on a button ("Ask Neovim") and
...
4. **The app only, no digest yet:** the raw history at the plugin's default
   ...
   [D1](#d1--the-plugin-publishes-a-digest-the-readers-do-not-re-derive-it).
...
only). A project imported from a URL already carries its `repo_url`
(`ProjectSettings`), which is tried first and costs no process. Three outcomes,
each shown as what it is:
...
| Repo | Change | Nature |
|---|---|---|
| `github_stats.nvim` | Write `digest/<repo>.json` and the `root.json` index after each fetch (atomic, changed-only, off the fetch path — [D1](#d1--the-plugin-publishes-a-digest-the-readers-do-not-re-derive-it)), into the configurable `digest_dir` (default under `stdpath("data")`); a UI-free `github_stats.digest` module exposing `digest_dir()`; the schema documented as a stable contract in `docs/FEATURES/` | small, additive |
| `documentation.nvim` (Neovim side) | `core/traffic_join.lua`: the discovery chain of D2, then reads the digest (decision 2 — not the live Lua API); a `traffic` browse mode beside `telemetry`/`rules`; `opts.traffic.digest_dir` | small, same pattern as `rules_join` |
...
| `docmap-desktop` | `traffic.rs` next to `telemetry.rs`: read pointer + digest, resolve project → repo, one Tauri command; a Traffic section with the folder button, the panel, and a column in the project list | medium |
...
3. **A Traffic section** — in the sidebar, next to the telemetry block (the app
   shell has no tab bar; telemetry sits in the sidebar, and the map's own tabs
   live inside the generated page), with a **detail dialog** after the pattern
   of the dependency matrix. It holds the folder button and what was found
   there, then the panel — a daily sparkline over the whole stored span (not just 14 days, which
...
- Its `User` events (`GitsuiteBranchSwitched`, …) are the sanctioned way to
  ...
  (`github_stats` refreshing its digest when a branch switch lands in a
...
|---|---|---|---|
| **P0** | Digest + `root.json` written after each fetch (atomic, changed-only) into a configurable `digest_dir`; a UI-free `github_stats.digest` exposes `digest_dir()`; the schema documented as a contract | `github_stats.nvim` | ~0.5 session |
...
| **P3** | `traffic_join.lua` and the `traffic` browse mode, live or via the digest | `documentation.nvim` | ~0.5–1 session |
...
The digest crosses a trust boundary the rest of the app does not: it is a JSON
...
- **The folder is read by Rust, not by the webview.** The chosen directory is
  ...
  result.
- **No secrets in it.** The digest carries counts and public strings only —
...
- **One read for the whole column**, in a single async Tauri command, cached by
  `(path, mtime)` — which is also why the plugin only rewrites a digest that
  changed (D1). Never on the UI thread, never one command per row.
- **Only the digest is read**, never the raw history, except in the labelled
  fallback of D1.
...
- **Schema drift.** Two readers, one writer. Version the digest (`schema`) and
...
- **Digest is per machine.** After a sync the local digest can lag the history
...
1. **History stays in the synced config dir; the derived digest and `root.json`
   go to a local, per-machine directory** — by default under `stdpath("data")`,
   overridable (decision 4; [D2](#d2--two-locations-history-synced-digest-local)).
   Reason: one dataset across PC and workstation, no sync conflicts on a
   rewritten file.
2. **The Neovim-side join reads the digest**, not the live Lua API — both
   consumers are checked against one contract.
3. **A per-project opt-out lives in the app**, as a plain per-project setting,
   not in the plugins.
4. **The digest location is configurable** (`digest_dir`), because users may
   already use `stdpath("data")` for their own data. Readers use one discovery
   chain: explicit setting (app: folder button in the Traffic section;
   `documentation.nvim`: `opts.traffic.digest_dir`) → `root.json` → asking
   Neovim (a UI-free `github_stats.digest` module; in the app, a button that
