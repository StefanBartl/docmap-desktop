...
| **L6** | **Have an agent run the checklists** — backend decided: loomAI | Two things to decide first, both about trust: a hand-written claim and a measured observation must not look alike, and an agent's edit is a *proposal*, not a result. Sizing and the loomAI-side state in [`AGENT_CHECKLIST_RUNNER.md`](AGENT_CHECKLIST_RUNNER.md). The rules-shaped half of the same idea is **L10**; the two share one store and one review pane, so build either without building the other's half twice |
...
| **L10** | **A rules view, and an agent for the manual rules** — concept only | `rules.nvim`'s catalog and mechanical gate in the desktop app, then the rules with no `check` handed to an agent whose answer is a *proposal*. Spans `rules.nvim`, `documentation.nvim`, this app and (optionally) loomAI; shares L6's store and review pane. Its own Rules tab with multi-select, automated and manual rules kept apart, a titled run in a window of its own, and a chat. **P0–P3 (~5–5.5 sessions) are useful with no agent in them**, through the run window ~9.5–10, the whole ~12–13; the real corpus is 92 % manual rules, which is why it batches. Design, decisions and the open questions in [`RULES_AGENT_CONCEPT.md`](RULES_AGENT_CONCEPT.md) |
| **L11** | **GitHub traffic in the explorer** — concept only | `github_stats.nvim`'s clones/views/referrers/paths as a Traffic line, a sortable project-list column and a panel; soft dependency, read-only, no token in the app. The plugin publishes a local digest (history stays in the synced config dir); the app and `documentation.nvim` read it. Repo-level data only — `paths` is GitHub's top 10, so no per-module heat map. `gitsuite.nvim` is not needed. **P0–P2 (~2.5 sessions) need no `documentation.nvim` change and are useful alone.** Design in [`GITHUB_STATS_CONCEPT.md`](GITHUB_STATS_CONCEPT.md) |
