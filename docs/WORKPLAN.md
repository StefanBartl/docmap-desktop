...
### 10.2 The theme still stops at the sidebar

Reported against the installed build, which *includes* the `?theme=` fix
(tagged at `3778fa2`, and the fix landed in `2885b83`).

- [ ] So this is not "unimplemented", it is "implemented and not working
      here", which is a different investigation. Check in order: does the
      installed engine's page carry the `?theme=` reader at all — the map
      may have been **generated by an older engine**, since the page is
      baked at generation time and `C:/tools/docmap.exe` is whatever was
      installed there; does `retheme()` fire from the Settings dialog's
      select; and does the frame actually reload.
- [ ] **The likely answer is the first one**, and it generalises: a map on
      disk is a snapshot of the engine that wrote it. Any page-side feature
      needs the map regenerated, and nothing in the app says so. That is
      worth surfacing on its own — see 10.7.

### 10.3 About says the wrong things, and one true thing badly

- [ ] `know your project` belongs on the title line next to the name,
      separated by a dash — `docmap — know your project` — not on a line of
      its own under it.
- [ ] The lead paragraph ("Ein Fenster vor den Karten, die
      documentation.nvim erzeugt. Zwei Programme…") was reported as not
      obviously belonging there. It is explaining *why the block below
      exists*, which is a thing to explain to somebody filing a bug and
      noise to everybody else. Cut it to one sentence, or drop it and let
      the copy button carry the meaning.
- [ ] **`grammars: //?/C:/Program Files/docmap-desktop/grammars` is a real
      bug**, visible in the screenshot. `resource_dir()` returns a
      `\?\`-prefixed path on Windows and `replace('\', "/")` turns the
      prefix into `//?/`. `add_project` already strips exactly this
      (`trim_start_matches(r"\?\")`); `resolve_grammars` does not.
- [ ] **Check whether that mangled path also reaches `DOCMAP_TS_DIR`**,
      because then it is not cosmetic: a `\?\` path with forward slashes is
      not a valid Windows extended path, and the failure mode would be an
      installed build silently producing module-tree-only maps. The
      screenshot shows four grammars loaded, so it apparently worked in that
      run — which means the two paths differ and the difference has to be
...
### 10.5 Sidebar spacing

- [ ] More air between the project badge and the collapsed Engine/Neovim
      rows.
- [ ] Those two rows pinned to the bottom of the sidebar rather than
      floating after the badge.
- [ ] A bigger badge is acceptable. It is the thing the sidebar is for.
